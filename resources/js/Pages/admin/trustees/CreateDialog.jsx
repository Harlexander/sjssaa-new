...
const CreateDialog = () => {
  const form = useForm({ name: '', position: '', email: '', image: null, order: '', is_active: true })
  const submit = (e) => { e.preventDefault(); form.post('/admin/trustees', { forceFormData: true }) }

  return (
    <Dialog>
      <DialogTrigger asChild>
        <button className='bg-[#800000] hover:bg-[#700000] text-white px-6 py-3 rounded-lg flex items-center gap-2'>
          <PlusIcon className='h-5 w-5'/> Add Trustee
        </button>
      </DialogTrigger>
      <DialogContent className='max-w-xl'>
        <DialogHeader>
          <DialogTitle>Add Trustee</DialogTitle>
          <DialogDescription>Fill in trustee details.</DialogDescription>
        </DialogHeader>
